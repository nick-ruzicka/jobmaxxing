...
| 2026-05-13 | `LocationTag`: NYC=emerald, Hybrid=blue, Remote=accent, other=neutral (kept the existing mapping, *not* the `/design-system` preview's NYC=blue) | The preview shows NYC=blue **and** Hybrid=blue, which collide. The preview is the bug; the running mapping has three distinct colors. Fix the preview's collision in commit 9. |
...
**REMAINING — start at 4b:**
4b. `refactor(pipeline): rebuild filter bar` — `FilterBar.tsx` rewrite: search (icon-prefixed, inline `×`, **no `focus:outline-none`**) + a score segmented control with real presence (~32px, `bg-surface-2 border rounded-md`, active = `bg-accent-dim text-accent`) + `{n} of {m}` count + a `<Button variant="ghost">` "Clear filters" (only when active) on row 1; chip groups (Location | Signals | Status | Aggregator) separated by **real `border-l border-border-subtle` dividers** on row 2 — each chip a small toggle whose count renders via `<Badge color="neutral">`, active chips get the accent treatment + a trailing `<X>` (so the active chips *are* the filter pills). `accent` for active/idle only (no per-category tint — color encodes on/off). `Filters` state stays in `PipelineTable`; FilterBar takes `onChange` + `onReset`.
