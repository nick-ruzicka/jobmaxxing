 ...
 * @param {(location: string) => string} [opts.bucketLocation] - location
 *   bucketing function (defaults to identity).
 * @returns {ChipCounts}
 */
export function computeChipCounts(roles, opts = {}) {
  const includeAggregator = opts.includeAggregator === true;
  const bucketLocation = typeof opts.bucketLocation === "function"
    ? opts.bucketLocation
    : (l) => l;
