...
test("computeChipCounts — default (includeAggregator=false) excludes aggregators", () => {
  const counts = computeChipCounts(FIXTURE, { bucketRole: (r) => bucketLocation(r.location) });
...
test("computeChipCounts — includeAggregator=true includes everything", () => {
  const counts = computeChipCounts(FIXTURE, { bucketRole: (r) => bucketLocation(r.location), includeAggregator: true });
...
test("computeChipCounts — aggCount counts over the full set even when toggle is off", () => {
  const offCounts = computeChipCounts(FIXTURE, { bucketRole: (r) => bucketLocation(r.location), includeAggregator: false });
  const onCounts  = computeChipCounts(FIXTURE, { bucketRole: (r) => bucketLocation(r.location), includeAggregator: true });
...
test("computeChipCounts — empty roles array → zeros, no throws", () => {
  const counts = computeChipCounts([], { bucketRole: (r) => bucketLocation(r.location) });
...
test("computeChipCounts — null entries in the array are skipped without throwing", () => {
  ...
  const counts = computeChipCounts(roles, { bucketRole: (r) => bucketLocation(r.location) });
  assert.equal(counts.statusCounts.Discovered, 1);
  assert.equal(counts.locBucketCounts.NYC, 1);
  assert.equal(counts.aggCount, 0);
});

test("computeChipCounts — falls back to r.location when no bucketRole passed", () => {
...
test("computeChipCounts — Boolean(includeAggregator) gate behaves strictly", () => {
  ...
  const truthy = computeChipCounts(FIXTURE, { bucketRole: (r) => bucketLocation(r.location), includeAggregator: "yes" });
  assert.equal(truthy.locBucketCounts.NYC, 2, "non-boolean truthy should not flip the gate");

  const undef = computeChipCounts(FIXTURE, { bucketRole: (r) => bucketLocation(r.location), includeAggregator: undefined });
