...
import {
  ...
  AlertTriangle, FileQuestion, Sparkles, ArrowUpDown, Building2, Hammer, Cpu,
} from "lucide-react";
...
.ds *::selection { background: var(--accent-dim); }
.ds h1,.ds h2,.ds h3 { color:var(--text-primary); }
...
.ds .t-section{font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:.06em;line-height:1;color:var(--text-tertiary);}
.ds .tnum{font-variant-numeric:tabular-nums;}
.ds .card{background:var(--surface-2);border:1px solid var(--border-subtle);border-radius:var(--r-lg);}
.ds .hr{height:1px;background:var(--border-subtle);border:0;}
...
.ds .badge-neutral{background:var(--surface-3);color:var(--text-tertiary);border-color:var(--border-subtle);}
.ds .badge-accent{background:var(--accent-dim);color:var(--accent);border-color:var(--accent-border);}
.ds .badge-emerald{background:var(--emerald-dim);color:var(--emerald);border-color:var(--emerald-border);}
.ds .badge-amber{background:var(--amber-dim);color:var(--amber);border-color:var(--amber-border);}
.ds .badge-blue{background:var(--blue-dim);color:var(--blue);border-color:var(--blue-border);}
.ds .badge-violet{background:var(--violet-dim);color:var(--violet);border-color:var(--violet-border);}
.ds .badge-red{background:var(--red-dim);color:var(--red);border-color:var(--red-border);}
...
.ds .btn:focus-visible{outline:none;box-shadow:0 0 0 2px var(--surface-0),0 0 0 4px var(--accent);}
.ds .btn-md{padding:8px 16px;}
.ds .btn-primary{background:var(--accent-strong);color:#fff;}
.ds .btn-primary:hover{background:#7077f2;}
.ds .btn-secondary{background:var(--surface-2);color:var(--text-secondary);border-color:var(--border-default);}
.ds .btn-secondary:hover{background:var(--surface-3);}
.ds .btn-ghost{background:transparent;color:var(--text-tertiary);}
.ds .btn-ghost:hover{background:var(--surface-3);color:var(--text-secondary);}
.ds .btn-danger{background:var(--red-dim);color:var(--red);border-color:var(--red-border);}
...
.ds .is-hover.btn-secondary,.ds .is-hover.btn-ghost{background:var(--surface-3);}
.ds .is-focus{box-shadow:0 0 0 2px var(--surface-0),0 0 0 4px var(--accent);}
.ds .is-active{transform:translateY(.5px);filter:brightness(.95);}

/* inputs */
.ds .input{background:var(--surface-2);border:1px solid var(--border-default);border-radius:var(--r-md);padding:8px 12px;font-size:13px;color:var(--text-secondary);width:100%;}
.ds .input::placeholder{color:var(--text-muted);}
.ds .input:focus-visible{outline:none;box-shadow:0 0 0 2px var(--surface-0),0 0 0 4px var(--accent);}

/* segmented control */
.ds .seg{display:inline-flex;gap:2px;background:var(--surface-2);border:1px solid var(--border-default);border-radius:var(--r-md);padding:3px;}
.ds .seg button{border:0;background:transparent;color:var(--text-muted);font-size:12px;font-weight:500;padding:5px 12px;border-radius:var(--r-sm);cursor:pointer;transition:background-color var(--dur) var(--ease-out),color var(--dur) var(--ease-out);}
.ds .seg button:hover{color:var(--text-secondary);}
.ds .seg button[aria-selected="true"]{background:var(--accent-dim);color:var(--accent);}

/* table */
.ds table{width:100%;border-collapse:collapse;font-size:13px;}
.ds .tbl{background:var(--surface-2);border:1px solid var(--border-subtle);border-radius:var(--r-lg);overflow:hidden;}
.ds thead tr{background:var(--surface-1);border-bottom:1px solid var(--border-subtle);}
.ds th{padding:10px 12px;text-align:left;font-size:11px;font-weight:500;text-transform:uppercase;letter-spacing:.04em;color:var(--text-tertiary);}
.ds tbody td{padding:10px 12px;border-bottom:1px solid var(--border-subtle);color:var(--text-secondary);}
.ds tbody tr:last-child td{border-bottom:0;}
.ds tbody tr:nth-child(even){background:var(--surface-row);}
.ds tbody tr.is-hover{background:var(--surface-3);}
.ds tbody tr.is-dimmed{opacity:.5;}
.ds tbody tr.is-focused td{position:relative;}
.ds tbody tr.is-focused{box-shadow:inset 0 0 0 1px var(--accent);}

/* nav item (sidebar) */
.ds .nav{display:flex;align-items:center;gap:10px;padding:7px 10px;border-radius:var(--r-md);font-size:13px;font-weight:500;color:var(--text-tertiary);border-left:2px solid transparent;}
.ds .nav:hover{background:var(--surface-3);color:var(--text-secondary);}
.ds .nav.is-selected{background:var(--accent-dim);color:var(--accent);border-left-color:var(--accent);}

/* skeleton */
@keyframes ds-pulse{0%,100%{opacity:1}50%{opacity:.45}}
.ds .skel{background:var(--surface-2);border-radius:var(--r-md);animation:ds-pulse 1.4s ease-in-out infinite;}

/* empty state */
.ds .empty{display:flex;flex-direction:column;align-items:center;text-align:center;padding:48px 16px;}

.ds .swatch{height:48px;border-radius:var(--r-md);border:1px solid var(--border-subtle);}
.ds .chip-key{font-size:11px;color:var(--text-muted);font-family:var(--font-mono,ui-monospace),monospace;}
.ds a.link{color:var(--accent);text-decoration:none;}
...
.ds .state-lbl{font-size:11px;color:var(--text-muted);text-align:center;}
`;
...
function Section({ id, label, title, desc, children }: { id: string; label: string; title: string; desc?: string; children: React.ReactNode }) {
  return (
    <section id={id} style={{ marginBottom: 56 }}>
      ...
      {desc && <p className="t-caption" style={{ color: "var(--text-muted)", marginBottom: 16, maxWidth: 680 }}>{desc}</p>}
      {children}
    </section>
  );
}
    ...
    <div className="ds">
      ...
      <div style={{ maxWidth: 920, margin: "0 auto", padding: "48px 32px 96px" }}>
        ...
        <header style={{ marginBottom: 48 }}>
          ...
          <p className="t-body" style={{ color: "var(--text-secondary)", maxWidth: 680 }}>
            ...
            <table>
              ...
              <tbody>
                {TYPE_ROWS.map(([tok, spec, sample]) => (
                  <tr key={tok}>
                    <td className="t-caption" style={{ color: "var(--text-muted)" }}>{tok}</td>
                    <td className="t-caption" style={{ color: "var(--text-tertiary)" }}>{spec}</td>
                    <td><span className={TYPE_CLASS[tok]} style={{ color: tok === "section-label" ? undefined : "var(--text-primary)" }}>{sample}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <Section id="color" label="Color" title="Surfaces & borders" desc="Four elevation steps you can barely tell apart, plus a zebra row tint. Three border weights. 1px borders do the elevation work — there are no shadows on cards or tables.">
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 12 }}>
            {SURFACES.map(([name, hex, use]) => (
              <div key={name} className="card" style={{ padding: 12 }}>
                ...
                <div className="t-body-strong" style={{ color: "var(--text-primary)" }}>{name}</div>
                <div className="chip-key">{hex}</div>
                <div className="t-caption" style={{ color: "var(--text-muted)", marginTop: 2 }}>{use}</div>
              </div>
            ))}
          </div>
          <div style={{ display: "flex", gap: 12, marginTop: 12 }}>
            {["border-subtle #1e1e2a", "border-default #26263a", "border-strong #32324a"].map((b) => {
              const [n, h] = b.split(" ");
              return <div key={n} style={{ flex: 1, height: 44, borderRadius: 8, border: `1px solid ${h}`, background: "var(--surface-1)", display: "flex", alignItems: "center", justifyContent: "center" }}>
                <span className="t-caption"><span style={{ color: "var(--text-secondary)" }}>{n}</span> <span className="chip-key">{h}</span></span>
              </div>;
            })}
          </div>
        </Section>
        ...
        <Section id="text" label="Color" title="Text tiers" desc="The two muted tiers were raised to meet WCAG AA 4.5:1 (old --text-tertiary ≈4.2:1, old --text-muted ≈2.5:1). Contrast ratios shown against surface-0.">
          <div className="card" style={{ padding: 16 }}>
            {TEXTS.map(([name, hex, ratio, use], i) => (
              <div key={name} style={{ display: "flex", alignItems: "baseline", gap: 16, padding: "10px 0", borderTop: i ? "1px solid var(--border-subtle)" : 0 }}>
                <span style={{ color: hex, fontSize: 16, fontWeight: 500, width: 220 }}>The quick brown fox — 12345</span>
                <span className="t-body-strong" style={{ color: "var(--text-primary)", width: 130 }}>{name}</span>
                <span className="chip-key" style={{ width: 80 }}>{hex}</span>
                <span className="t-caption" style={{ color: ratio.includes("FAIL") ? "var(--red)" : "var(--emerald)", width: 90 }}>{ratio}</span>
        ...
        <Section id="semantics" label="Color" title="Semantic palette" desc="Each color owns a meaning. Score tiers and status colors draw from this set — indigo is no longer overloaded. Every semantic comes with -dim (12% fill) and -border (25% stroke) tokens so components stop hand-deriving rgba(...,0.2) literals.">
          {SEMANTICS.map(([name, hex, means, use]) => (
            <div key={name} className="card" style={{ padding: 14, marginBottom: 10, display: "flex", alignItems: "center", gap: 16 }}>
              ...
              <span className="t-caption" style={{ color: "var(--text-secondary)", width: 200, flexShrink: 0 }}>{means}</span>
              <span className="t-caption" style={{ color: "var(--text-muted)" }}>{use}</span>
            </div>
          ))}
        </Section>
        ...
        <Section id="spacing" label="Spacing" title="Spacing scale" desc="4px base, aligned with Tailwind defaults so utilities map 1:1. 2.5 (10px) is a named exception used only for dense table cell padding (gives ~40–42px rows). Snap everything else to the scale.">
          <div className="card" style={{ padding: 16 }}>
            {SPACING.map(([name, px, use], i) => (
              <div key={name} style={{ display: "flex", alignItems: "center", gap: 16, padding: "8px 0", borderTop: i ? "1px solid var(--border-subtle)" : 0 }}>
                <span className="t-body-strong tnum" style={{ color: "var(--text-primary)", width: 32 }}>{name}</span>
                <span className="t-caption tnum" style={{ color: "var(--text-muted)", width: 48 }}>{px}px</span>
                <div style={{ width: px, height: 14, background: name === "2.5" ? "var(--amber)" : "var(--accent)", borderRadius: 2 }} />
                <span className="t-caption" style={{ color: "var(--text-muted)" }}>{use}</span>
              </div>
            ))}
          </div>
        ...
        <Section id="buttons" label="Components" title="Button" desc="primary / secondary / ghost / danger × sm/md. Rounded-md, 13px medium, gap-1.5, explicit transitions (no transition:all), :active translateY(0.5px), disabled = opacity-40 + not-allowed, focus-visible = the one global ring.">
          <div className="card" style={{ padding: 20 }}>
            ...
            <div style={{ display: "flex", gap: 12, alignItems: "center", maxWidth: 520 }}>
              ...
              <div className="seg">
                <button>All</button><button>4+</button><button aria-selected="true">6+</button><button>8+</button>
              </div>
            </div>
          </div>
        </Section>
          ...
          <div className="card" style={{ padding: 20 }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 16, marginBottom: 20 }}>
              {[["neutral", "Discovered"], ["accent", "Evaluated"], ["violet", "Applied"], ["blue", "Interview"], ["emerald", "Offer"], ["red", "Rejected"]].map(([c, l]) => (
                <span key={l} className="t-small" style={{ display: "inline-flex", alignItems: "center", gap: 7, color: "var(--text-secondary)" }}>
                  <span className="dot" style={{ background: `var(--${c})` }} />{l} <ChevronDown size={11} style={{ color: "var(--text-muted)" }} />
                </span>
              ))}
            </div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 24, alignItems: "center" }}>
              {[["9", "emerald", "emerald", "AI-enriched"], ["7", "blue", "neutral", "title-only"], ["6", "blue", "blue", "manual"], ["5", "amber", "violet", "override"]].map(([n, sc, pr, lbl]) => (
                <span key={lbl} style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
                  ...
                  <span className="t-caption" style={{ color: "var(--text-muted)" }}>{lbl}</span>
                </span>
              ))}
            </div>
            <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
              <span className="t-small" style={{ display: "inline-flex", alignItems: "center", gap: 6, color: "var(--emerald)" }}><span className="dot" style={{ background: "var(--emerald)" }} />Build component</span>
              <span className="t-small" style={{ display: "inline-flex", alignItems: "center", gap: 6, color: "var(--emerald)" }}><span className="dot" style={{ background: "var(--emerald)" }} />AI signal</span>
              <span className="t-small" style={{ display: "inline-flex", alignItems: "center", gap: 6, color: "var(--amber)" }}><span className="dot" style={{ background: "var(--amber)" }} />No build component</span>
            </div>
          </div>
        </Section>
        ...
        <Section id="table" label="Components" title="DataTable" desc="One shell — border + rounded-lg + overflow-x-auto, no shadow. Uppercase tracked column heads. ~40px rows. Zebra and hover are CSS-only (even:bg-surface-row, hover:bg-surface-3) — zero JS hover handlers. Dimmed rows = opacity-50 only, no strikethrough, no left-border. Keyboard-focused row uses the global ring.">
            ...
            <table>
              ...
              <tbody>
                {[
                  ...
                  { s: "8", sc: "emerald", pr: "blue", co: "Mutiny", ro: "GTM Engineer", comp: "$150–200k", loc: ["blue", "NYC"], st: ["accent", "Evaluated"], f: "2mo", cls: "" },
                  { s: "7", sc: "blue", pr: "violet", co: "Hebbia", ro: "GTM Engineer", comp: "—", loc: ["blue", "NYC"], st: ["blue", "Interview"], f: "1mo", cls: "is-focused" },
                  ...
                  <tr key={i} className={r.cls}>
                    ...
                    <td><span className="t-body-strong" style={{ color: "var(--text-primary)" }}>{r.co}</span></td>
                    <td>{r.ro}</td>
                    <td className="tnum t-caption" style={{ color: r.comp === "—" ? "var(--text-muted)" : "var(--text-secondary)" }}>{r.comp}</td>
                    <td><span className={`badge badge-${r.loc[0]}`}><Building2 size={10} />{r.loc[1]}</span></td>
                    <td><span className="t-small" style={{ display: "inline-flex", alignItems: "center", gap: 6 }}><span className="dot" style={{ background: `var(--${r.st[0]})` }} />{r.st[1]} <ChevronDown size={10} style={{ color: "var(--text-muted)" }} /></span></td>
                    <td className="t-micro" style={{ color: "var(--text-muted)" }}>{r.f}</td>
          ...
          <p className="t-caption" style={{ color: "var(--text-muted)", marginTop: 8 }}>Row 2 = hover · Row 4 = keyboard-focused · Row 5 = dimmed (skipped)</p>
        </Section>
        ...
        <Section id="nav" label="Components" title="Sidebar nav & SectionLabel" desc="Selected = accent-dim fill + 2px left accent bar (the only place a left-accent bar is allowed — it's wayfinding, not card decoration). The logo is a monochrome icon, not a tinted circle. Group headers use the tracked-uppercase SectionLabel.">
          <div className="card" style={{ padding: 16, maxWidth: 260 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "4px 6px", marginBottom: 16 }}>
              <Briefcase size={16} style={{ color: "var(--text-primary)" }} />
              <span className="t-heading">JobOps</span>
              <Bell size={14} style={{ color: "var(--text-muted)", marginLeft: "auto" }} />
            </div>
        ...
        <Section id="states" label="Patterns" title="Empty / loading / error" desc="Empty = centered icon + title + optional description + optional secondary-button action. Loading = skeleton blocks that match the real layout (opacity pulse, not a moving gradient). Error = the empty pattern with AlertTriangle + a 'Try again' button.">
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <div className="card empty">
              ...
              <div className="t-small" style={{ color: "var(--text-secondary)" }}>No roles match your filters</div>
              <div className="t-caption" style={{ color: "var(--text-muted)", marginTop: 4 }}>Try widening the score range or clearing location filters.</div>
            ...
            <div className="card empty">
              ...
              <div className="t-small" style={{ color: "var(--text-secondary)" }}>Something went wrong</div>
              <div className="t-caption" style={{ color: "var(--text-muted)", marginTop: 4 }}>Failed to read the pipeline data.</div>
        ...
        <Section id="motion" label="Motion" title="Transitions" desc="">
          <div className="card" style={{ padding: 16 }}>
            <ul className="t-small" style={{ color: "var(--text-secondary)", lineHeight: 1.7, paddingLeft: 18, margin: 0, listStyle: "disc" }}>
        ...
        <div className="bg-surface-2 text-text-primary border border-border-subtle rounded-md" style={{ padding: "8px 12px", marginBottom: 24, display: "flex", alignItems: "center", gap: 8 }}>
          <span className="t-caption" style={{ color: "var(--text-muted)" }}>@theme smoke test — this row is styled with Tailwind utilities:</span>
