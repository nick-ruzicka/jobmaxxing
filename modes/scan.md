...
7.5. **Verify liveness of WebSearch results (Level 3)** — BEFORE adding to pipeline:
   ...
   WebSearch results may be outdated (Google caches results for weeks or months). To avoid evaluating expired offers, verify each new URL from Level 3 with Playwright. Levels 1 and 2 are inherently real-time and don't require this verification.
   ...
   **Don't interrupt the entire scan if a URL fails.** If `browser_navigate` gives an error (timeout, 403, etc.), mark as `skipped_expired` and continue with the next one.
...
Each company in `tracked_companies` should have `careers_url` — the direct URL to their job listings page. This avoids searching for it each time.
